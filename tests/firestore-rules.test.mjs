...
await seed('accountProfiles/player',{role:'player'});
...
await expect('/tables/main','GET',p,null,403);
await expect(':commit','POST',p,commit('tables/main',{stateJson:'{}',revision:2},'updatedAt'),403);
...
await expect('/tables/main','GET',a,null,403);
console.log('PASS: Firestore rules reject strangers, wrong hashes, role injection, other sessions, players, revoked and expired sessions; valid admin login/write/logout succeeds.');
